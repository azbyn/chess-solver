...
abstract class SlidersViewModel: BaseViewModel() {
    ...
    fun fastForward(frag: BaseFragment, p: IntArray): Boolean {
    ...
    @CallSuper
    protected open fun update(args: IntArray, isFastForward: Boolean) {
        for ((i, prog) in args.withIndex()) {
            lastValues[i] = prog
        }
    }
    ...
    open fun saveData(sliderDatas: Array<SliderData>) = JSONObject().apply {
...
abstract class DumbSlidersFragment(
    ...
    final override fun getImageView(): ZoomableImageView = imageView!!
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        for ((i, s) in sliderDatas.withIndex()) {
            ...
            view.findViewById<View>(minusIndices[i]).setOnClickListener {
                --sb.progress
                humanModified = true
            }
            view.findViewById<View>(plusIndices[i]).setOnClickListener {
                ++sb.progress
                humanModified = true
            }
        }
