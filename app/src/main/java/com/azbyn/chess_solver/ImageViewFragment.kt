...
abstract class ImageViewFragment: BaseFragment() {
    ...
    fun setImageGrayscalePreview(src: Mat, imageView: ImageView = getImageView()) =
        imageView.setImageBitmap(previewViewModel.grayscaleMatToPreview(src))

    fun setImagePreview(src: Mat, imageView: ImageView = getImageView()) {
        if (src.width() == 0 && src.height() > 0) return

        imageView.setImageBitmap(previewViewModel.matToPreview(src))
    }
